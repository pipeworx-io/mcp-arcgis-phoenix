 ...
 *
 * City of Phoenix GIS (ArcGIS org cfKakmeHE95cgeEK): parcels, zoning, public works & city services. search_datasets finds layers (each
...
const UA = 'pipeworx-mcp-arcgis-phoenix/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search City of Phoenix GIS open geospatial datasets (parcels, zoning, public works & city services) by keyword. Returns each dataset\'s name, summary, record_count, owner/org, and its Feature Service `url` — pass that url to query_layer / layer_info.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword(s), e.g. "parcels", "crime", "flood zones".' },
        org_id: { type: 'string', description: 'Optional ArcGIS orgId to override the default (City of Phoenix GIS).' },
